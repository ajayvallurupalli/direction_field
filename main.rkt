...
(define (trajectory vector matrix iterations total-iterations dc)
  (define next (matrix* matrix vector))
  ...
  (send dc draw-text
        (number->string (- total-iterations iterations -1))
        (+ (first vector-pair) (/ WIDTH 2))
        (-  (/ HEIGHT 2) (second vector-pair)))

  (unless (= iterations 0)
    (trajectory next matrix (sub1 iterations) total-iterations dc)))
